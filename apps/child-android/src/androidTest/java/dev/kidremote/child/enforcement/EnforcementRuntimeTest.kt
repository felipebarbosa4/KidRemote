...
class EnforcementRuntimeTest {
 @Test fun serviceLifecycle(){
    ...
    try {
        ...
        IdentityStore(c).save(JSONObject().put("device_id",UUID.randomUUID().toString()).put("policy_epoch",epoch).put("credential",java.util.Base64.getUrlEncoder().withoutPadding().encodeToString(ByteArray(32).also{java.security.SecureRandom().nextBytes(it)})))
        val sample=AndroidAccountingClock.sample(c,true)
        val utc=java.time.Instant.parse("2026-09-14T12:00:00Z").toEpochMilli()
        ChildAccounting(c).use{check(!it.initialize(Policy(epoch,1,"Etc/UTC",1,"2026-09-14",3600,600,true),sample,utc).storageFailure)}
