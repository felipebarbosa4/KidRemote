...
class RecoveryTest {
    ...
    private fun legacy(s:Ledger):ByteArray {val body=LedgerCodec.encode(s).dropLast(24).toByteArray();java.nio.ByteBuffer.wrap(body).putInt(1);return ByteArrayOutputStream().also{it.write(body);DataOutputStream(it).writeLong(CRC32().apply{update(body)}.value)}.toByteArray()}
