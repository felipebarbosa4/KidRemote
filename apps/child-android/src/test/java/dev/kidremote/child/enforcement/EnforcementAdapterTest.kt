...
class EnforcementAdapterTest {
 ...
 @Test fun oldV3RetainsLedgerWithoutRestoringApplication(){val s=state();assertEquals(s,LedgerCodec.decode(LegacyFixture.encode(s,3)));assertNull(LedgerCodec.decode(LegacyFixture.encode(s,3)).lastAdapterObservation)}
 @Test fun blockedIntervalDoesNotLatchDesiredRestrictionOrConsume(){val s=state().copy(signals=state().signals.copy(blocked=true));val n=Accounting.sample(s,Sample(1,1000,1000,s.signals));assertEquals(0,n.usedMs);assertFalse(n.restrictionRequired);assertEquals(n,LedgerCodec.decode(LedgerCodec.encode(n)))}
 @Test fun transitionalV4RemainsReadable(){val s=state().copy(lastAdapterObservation="past-observation");assertEquals(s,LedgerCodec.decode(LegacyFixture.encode(s,4)))}
}
