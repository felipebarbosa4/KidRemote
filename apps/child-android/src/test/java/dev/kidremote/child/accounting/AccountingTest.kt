...
class AccountingTest {
    ...
    @Test fun strongUncertaintySurvivesResume(){for(u in listOf(Uncertainty.CLOCK,Uncertainty.STORAGE)){val s=start().copy(uncertainty=u);assertEquals(s.copy(recoveryThrough=if(u==Uncertainty.STORAGE)1000 else 0),Accounting.resume(s,at(1000)))}}
    ...
    @Test fun knownPrefixOnly(){val n=Accounting.reconcile(start(),ranges(Range(7,0,1000,yes),Range(7,2000,3000,yes)),at(3000),true);assertEquals(0,n.usedMs);assertEquals(0,n.cursor);assertTrue(n.restrictionRequired)}
    ...
    @Test fun invalidRecoveryClockCannotAdvanceMidnight(){val s=start(utc="2026-09-13T23:59:59Z");val n=Accounting.reconcile(s,ranges(Range(7,0,2000,yes)),at(2000,uptime=3000),true);assertEquals(s.copy(uncertainty=Uncertainty.CLOCK),n)}
