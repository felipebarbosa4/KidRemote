...
internal class ChildAccounting(private val context:Context):AutoCloseable {
    ...
    private fun syncAfterRecovery(result:AccountingResult){if(!result.storageFailure&&result.ledger?.uncertainty==Uncertainty.NONE&&dev.kidremote.child.sync.SyncFaults.automaticAllowed(context))dev.kidremote.child.sync.SyncRecovery.notify(context)}
