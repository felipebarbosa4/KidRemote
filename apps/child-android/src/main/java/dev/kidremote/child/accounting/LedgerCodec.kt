...
object LedgerCodec {
    fun encode(s:Ledger):ByteArray {
        ...
        DataOutputStream(bytes).use { o->
            o.writeInt(3);with(s.policy){o.writeUTF(epoch);o.writeLong(version);o.writeUTF(zone);o.writeLong(zoneRevision);o.writeUTF(bonusDate);o.writeLong(dailyLimitSeconds);o.writeLong(bonusSeconds);o.writeBoolean(manualLock)}
            ...
            o.writeUTF(s.uncertainty.name);o.writeBoolean(s.previousDate!=null);s.previousDate?.let{o.writeUTF(it)};o.writeLong(s.previousUsedMs);o.writeLong(s.recoveryThrough);o.writeLong(s.observedBoot);o.writeLong(s.reportSequence);o.writeBoolean(s.pendingAck!=null);s.pendingAck?.let{o.writeUTF(it)}
        }
    ...
    fun decode(bytes:ByteArray):Ledger {
        ...
        return DataInputStream(ByteArrayInputStream(body)).use { i->
            val format=i.readInt();require(format in 1..3)
            ...
            Ledger(p,date,used,bonus,boot,cursor,uptime,ae,au,signals,health,previous,previousUsed,through,observed,sequence,ack).also{it.validate()}
        }
    }
}
