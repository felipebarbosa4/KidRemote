...
object LedgerCodec {
    fun encode(s:Ledger):ByteArray {
        ...
        DataOutputStream(bytes).use { o->
            o.writeInt(4);with(s.policy){o.writeUTF(epoch);o.writeLong(version);o.writeUTF(zone);o.writeLong(zoneRevision);o.writeUTF(bonusDate);o.writeLong(dailyLimitSeconds);o.writeLong(bonusSeconds);o.writeBoolean(manualLock)}
    ...
    fun decode(bytes:ByteArray):Ledger {
        ...
        return DataInputStream(ByteArrayInputStream(body)).use { i->
            val format=i.readInt();require(format in 1..4)
