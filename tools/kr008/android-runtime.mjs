...
try {
 ...
 for(const [i,file] of ['child-debug.apk','child-debug-androidTest.apk'].entries()) {
  const path=join(dir,process.env.KR008_APK_BUNDLE??'apks-kr008-accounting',file);evidence.hashes[file]=createHash('sha256').update(readFileSync(path)).digest('hex');
