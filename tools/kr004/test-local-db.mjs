...
if (options.length > 3 || (options.length===3 && !['--pairing','--parent-test','--parent-dev','--parent-runtime','--enrollment-test','--enrollment-runtime','--enrollment-dev','--sync-test','--sync-runtime'].includes(options[2])) || !localEndpoints.includes(host))
...
const parentMode=options[2]?.startsWith('--parent-') || options[2]?.startsWith('--enrollment-') || options[2]?.startsWith('--sync-');
...
try {
  ...
  if(parentMode) {
    ...
    await runParentAuth({sql,call,env,name,label,token,network,databaseHost:name,
      ...
      enrollment:options[2]?.startsWith('--enrollment-')||options[2]?.startsWith('--sync-'),sync:options[2]?.startsWith('--sync-'),syncRuntime:options[2]==='--sync-runtime',enrollmentRuntime:options[2]==='--enrollment-runtime',gatewayConfig:{docker,host,container:id,label,owner:token}});
  }
} catch (error) {
  primary = error;
