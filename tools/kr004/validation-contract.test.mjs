...
test('recorded AC-1–4 assertions remain unchanged except explicit AC-6/OD-47 table inventory increments',()=>{
  for(const [path,hash] of [
    ...
    if(path.endsWith('01_rls.test.sql')) {
      assert.ok(source.includes('count(*) = 17 and bool_and(c.relrowsecurity)'));
      assert.ok(source.includes('all 17 app tables enable RLS'));
      source=source.replace('count(*) = 17 and bool_and(c.relrowsecurity)','count(*) = 15 and bool_and(c.relrowsecurity)').replace('all 17 app tables enable RLS','all 15 app tables enable RLS');
    }
