...
try{
    ...
    if(Test-Path -LiteralPath $runDirectory){
        try{Invoke-Cleanup}catch{if($script:Status -eq 'PENDING_SUCCESS'){$script:Status='FAIL';$script:Reason='CLEANUP_NOT_VERIFIED'}}
        ...
        try{Write-CalibrationJson 'summary.json' $summary}catch{}
