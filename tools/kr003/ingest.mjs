...
export function ingestQualification(directory) {
  ...
  const manifest = read("manifest.json"), rows = read("attempts.json"), summary=read("summary.json");
  assert.equal(manifest.Bundle.protocol,"KR003-Q1");
  ...
  if(qualified) {
    assert.equal(rows.length,100,"No hidden invalid/failed attempt allowed in a qualified run");
