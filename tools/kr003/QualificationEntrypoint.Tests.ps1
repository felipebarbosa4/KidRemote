<#
Goal: Execute the real configuration-bound qualification entrypoint initialization without reaching ADB.
Context: Native engine execution catches top-level PowerShell collisions and binding defects missed by function-only tests.
Constraints: Redirected input must stop before output-directory creation, bundle reads, radio changes or device commands.
...
$temporaryRoot=Join-Path ([IO.Path]::GetTempPath()) ('kr003-qualification-entrypoint-'+[Guid]::NewGuid().ToString('N'))
try{
    ...
    $start.Arguments='-NoProfile -NonInteractive -ExecutionPolicy Bypass -File "'+$entrypoint+'" -OfflineNetwork -Adb "'+$adb+'" -OutputRoot "'+$outputRoot+'"'
    ...
    Write-Host ($script:Checks.ToString()+' qualification-entrypoint startup assertions passed under PowerShell '+$PSVersionTable.PSVersion.ToString()+'; no device command was executed.')
}finally{
