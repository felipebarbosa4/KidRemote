...
for (const module of ["app","ordinary-fixture"]) {
  for (const variant of ["debug","release"]) {
    ...
    if (variant==="release") {
      assert(!/LabControlReceiver|FixtureReceiver|android.permission.DUMP|android:debuggable="true"/.test(manifest), "Release manifest contains lab access");
    } else {
    ...
    if (variant==="release") {
      for (const name of ["LabControlReceiver","FixtureReceiver","LabProbe","TraceJournal","KidRemoteKR003","KR003:"]) {
        assert(!dex.includes(Buffer.from(name)), "Release DEX contains " + name);
      }
    } else {
      assert(dex.includes(Buffer.from(module==="app" ? "LabControlReceiver" : "FixtureReceiver")), "Debug control absent");
    }
