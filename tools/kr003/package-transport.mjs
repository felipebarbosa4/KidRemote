// Goal: produce an immutable Q7 oracle-transport preflight bundle from a clean source commit.
// Context: three Q7 INVALID runs localize failure to the unblocked fixture input tap but do not retain its safe stderr class.
...
execFileSync(resolve(spike,'gradlew'),['--no-daemon',':ordinary-fixture:lintDebug',':ordinary-fixture:assembleDebug',':ordinary-fixture:lintRelease',':ordinary-fixture:assembleRelease'],{cwd:spike,stdio:'inherit'});
...
const mapping={'Test-KR003-OracleTransport.ps1':'tools/kr003/Test-KR003-OracleTransport.ps1','OracleTransport.psm1':'tools/kr003/OracleTransport.psm1','Qualification.psm1':'tools/kr003/Qualification.psm1','protocol.md':'docs/test-plans/KR-003-Q7-ORACLE-TRANSPORT-PREFLIGHT.md','ordinary-fixture.apk':'spikes/android-enforcement/ordinary-fixture/build/outputs/apk/debug/ordinary-fixture-debug.apk'};
...
const manifest={schema:1,protocol:'KR003-Q7-ORACLE-TRANSPORT-PREFLIGHT',sourceCommit:commit,runnerVersion:1,diagnosticOnly:true,physicalExecution:'NOT_RUN',files,fixtureSha256:files.find(f=>f.name==='ordinary-fixture.apk').sha256,ownerDeviceClass:{model:'Xiaomi Mi 8',miui:'MIUI Global 12.0.3',api:29,codename:'dipper'}};
