<#
Goal: Test one UiAutomation touch through separate self-targeted instrumentation and an independent fixture counter.
...
#>
param([string]$Adb='C:\platform-tools\adb.exe',[string]$OutputRoot='C:\platform-tools\kr003-uiautomation-transport')
...
try {
    ...
    if ($script:Bundle.schema -ne 1 -or $script:Bundle.protocol -ne 'KR003-UIAUTOMATION-TRANSPORT-PREFLIGHT' -or $script:Bundle.runnerVersion -ne 1 -or -not $script:Bundle.diagnosticOnly) { throw 'INVALID:BUNDLE_SCHEMA' }
    ...
    $null=Invoke-TransportAdb 'PROBE_INSTALL' @('install','-r',(Join-Path $PSScriptRoot 'input-probe.apk'))
    ...
    $probeRaw=Invoke-TransportAdb 'UIAUTOMATION_TAP' @('shell','am','instrument','-w','-r','-e','x',([string]$before.probeX),'-e','y',([string]$before.probeY),'-e','request',([string]$probeRequest),'dev.kidremote.spike.inputprobe/.OneTouchInstrumentation')
    $script:ProbeResult=Convert-KRUiAutomationReply $probeRaw $probeRequest
    ...
    if ($script:ProbeResult.Outcome -ne 'INJECTED' -or $script:ProbeResult.Cleanup -ne 'FRAMEWORK_FINISH' -or -not $script:ProbeResult.DownAccepted -or -not $script:ProbeResult.UpAccepted) {
        throw 'INVALID:UIAUTOMATION_INJECTION_OR_CLEANUP'
    }
    if (-not $after.focused -or -not $after.resumed) { throw 'INVALID:FIXTURE_LOST_FOCUS' }
    ...
    if (Test-Path -LiteralPath $runDirectory) {
        ...
        $summary=[PSCustomObject]@{
            Protocol='KR003-UIAUTOMATION-TRANSPORT-PREFLIGHT'; Status=$script:Status; Reason=$script:Reason
            SourceCommit=$(if($null -eq $script:Bundle){$null}else{$script:Bundle.sourceCommit})
            ProbeSha256=$(if($null -eq $script:Bundle){$null}else{$script:Bundle.probeSha256})
