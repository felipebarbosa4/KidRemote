<#
Goal: Isolate whether the Mi 8 accepts the one real ADB tap required by the Q7 independent oracle.
Context: Three Q7 attempts reached the focused fixture and failed before ARM with coarse INVALID:ADB_REJECTED.
...
#>
param([string]$Adb='C:\platform-tools\adb.exe',[string]$OutputRoot='C:\platform-tools\kr003-oracle-transport')
...
try {
    ...
    if ($script:Bundle.schema -ne 1 -or $script:Bundle.protocol -ne 'KR003-Q7-ORACLE-TRANSPORT-PREFLIGHT' -or $script:Bundle.runnerVersion -ne 1 -or -not $script:Bundle.diagnosticOnly) { throw 'INVALID:BUNDLE_SCHEMA' }
    ...
    $null=Invoke-TransportAdb 'INPUT_TAP' @('shell','input','tap',([string]$before.probeX),([string]$before.probeY))
    $watch=[Diagnostics.Stopwatch]::StartNew()
    do {
    ...
    if (Test-Path -LiteralPath $runDirectory) {
        ...
        $summary=[PSCustomObject]@{
            Protocol='KR003-Q7-ORACLE-TRANSPORT-PREFLIGHT'; Status=$script:Status; Reason=$script:Reason
