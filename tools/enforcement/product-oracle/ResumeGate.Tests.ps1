...
try{
 foreach($case in @('EMPTY','LOCAL_ONLY','PENDING','BACKEND_ONLY','REUSE','METADATA_UNKNOWN','UNKNOWN','ORPHAN','OLD_PACKAGE','FOREIGN_EPOCH','POLICY_AMBIGUOUS')){
  ...
  $metadata=[pscustomobject]@{status=$(if($case -ceq 'METADATA_UNKNOWN'){'UNKNOWN'}else{'METADATA_ONLY'});otherDurableFiles=$(if($case -ceq 'UNKNOWN'){1}else{0});files=@([pscustomobject]@{kind='identity';presence=$(if($case -cin @('LOCAL_ONLY','REUSE','FOREIGN_EPOCH')){'PRESENT'}else{'ABSENT'})},[pscustomobject]@{kind='pairing';presence=$(if($case -ceq 'PENDING'){'PRESENT'}else{'ABSENT'})})}
  ...
  $expected=switch($case){'EMPTY'{'ENROLL'} 'REUSE'{'VERIFY_REUSE'} {$_ -in @('METADATA_UNKNOWN','UNKNOWN','ORPHAN','OLD_PACKAGE','FOREIGN_EPOCH','POLICY_AMBIGUOUS')}{'NONE'} default{'RESET_ENROLL'}}
  $reason=switch($case){'METADATA_UNKNOWN'{'METADATA_KNOWN'} 'UNKNOWN'{'NO_UNKNOWN_FILES'} 'ORPHAN'{'SAVED_DEVICE_ORPHANED'} 'OLD_PACKAGE'{'PACKAGE_PROVENANCE'} {$_ -in @('FOREIGN_EPOCH','POLICY_AMBIGUOUS')}{'POLICY_STATE_AMBIGUOUS'} default{'NONE'}}
