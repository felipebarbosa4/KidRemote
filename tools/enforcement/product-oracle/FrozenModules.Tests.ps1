...
$names=@('ResumeReview','ResumePreparation','Reuse','BackendHost','Journal','Replacement','ReplacementAdb','ReadOnly','Canonical','EnrollmentHost','QrPresentation','LivePreparation','LiveSlice','ProductOracle','ProductTransport')
try{
 ...
 foreach($command in @('Resolve-ProductPreparation','Invoke-ResumePreparation','Read-ResumeReview','Clear-ProductLabSavedDevice','New-ProductJournal','Read-ProductJournal','Add-ProductJournal','Invoke-ProductHostGate','Start-ProductBackend','Invoke-ReviewProcess','New-LivePreparation','New-LiveSliceCallbacks','Invoke-ProductSlice','New-ProductQrWindow','Invoke-ProductQrEnrollment')){
 ...
 try{
  $prep=New-LivePreparation 'SYNTHETIC_ADB' '' $root $root $jwt
 ...
 Write-Output 'FROZEN_OWNER_MODULE_IMPORTS=16;ENTRYPOINT_PARSE=PASS;COMMANDS=15;ENROLL_CLOSURE=PASS;JOURNAL_CREATE_READ=PASS;DEVICE=NOT_INVOKED'
}finally{
