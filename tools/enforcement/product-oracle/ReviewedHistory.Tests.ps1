...
function New-ReviewedFixture([string[]]$Stages){
 ...
 [IO.File]::WriteAllText((Join-Path $dir 'result.txt'),(@{hostValidated=$true;hostFailureCode='SANITIZED_HOST_EXCEPTION';primary=@{reason='INVALID:HOST_OR_TRANSPORT_FAILURE';cleanup='UNVERIFIED'};reverseCleanup='OWN_REVERSE_REMOVED';backendCleanup='STOPPED_SYNTHETIC_LEASE_AND_ENROLLMENT_RETAINED'}|ConvertTo-Json -Depth 5))
 ...
 [IO.File]::WriteAllText($catalog,(@{schema=1;reviews=@(@{attempt=$meta.attempt;source=$meta.source;bundle=$meta.bundle;child=$meta.child;fixture=$meta.fixture;stages=$Stages;primaryReason='INVALID:HOST_OR_TRANSPORT_FAILURE';hostFailureCode='SANITIZED_HOST_EXCEPTION';pairingSession=@{id=[Guid]::NewGuid().ToString();disposition='OPEN'};inventory=$inventory})}|ConvertTo-Json -Depth 8))
