...
import {Lease,dockerCall,atomic,check,parsePrivateFrame} from './lease.mjs';
let input='';process.stdin.on('data',b=>{input+=b;});process.stdin.on('end',()=>{
 try{
  check(input.length<16384,'ADMIN_BOUNDS');const c=parsePrivateFrame(input);check(['status','teardown'].includes(c.action),'ADMIN_ACTION');
