...
if(count!==33)throw Error('CHILD_TEST_COUNT');
for(const variant of ['debug','release']){
 ...
 if(permissions.some(p=>!['android.permission.INTERNET','android.permission.CAMERA',id+'.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION',...(variant==='debug'?['android.permission.PACKAGE_USAGE_STATS']:[])].includes(p))||!manifest.includes('android:allowBackup="false"')||!manifest.includes('android:dataExtractionRules='))throw Error('CHILD_PERMISSION_BACKUP_SCOPE');
