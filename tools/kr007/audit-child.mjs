...
for(const variant of ['debug','release']){
 ...
 if(permissions.some(p=>!['android.permission.INTERNET','android.permission.CAMERA','android.permission.ACCESS_NETWORK_STATE','android.permission.WAKE_LOCK','android.permission.RECEIVE_BOOT_COMPLETED',id+'.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION',...(variant==='debug'?['android.permission.PACKAGE_USAGE_STATS']:[])].includes(p))||!manifest.includes('android:allowBackup="false"')||!manifest.includes('android:dataExtractionRules='))throw Error('CHILD_PERMISSION_BACKUP_SCOPE');
 ...
 if(variant==='release'&&/setAfterPage|sync-test-control|setBeforeRecoveryCommit|setAfterRecoveryCommit|EXPECTED_KILL_RECOVERY|setAfterPersist|setAfterAckResponse|setTransformSyncResponse|KR009_EXPECTED_KILL/.test(dex))throw Error('CHILD_RELEASE_RECOVERY_FAULT');
