...
export async function testEnrollment({http,sql}) {
 ...
 sql(`update private.device_credentials set revoked_at=clock_timestamp() where device_id='${device}';`);
 const retired=await read();ok(retired.status===403&&JSON.stringify(retired.json)==='{"code":"CREDENTIAL_REVOKED"}','RETIRED_GENERATION_NOT_DEVICE_REMOVAL');
 sql(`update private.device_credentials set revoked_at=null where device_id='${device}';`);
 ok(finish(q.qr.session_id,true).result==='REVOKED_FRESH_QR_REQUIRED','RECOVERY_REVOKES_INCOMPLETE');
 const removed=await read();ok(removed.status===403,'REVOKED_CURRENT_RECORD');
 ok(JSON.stringify(Object.keys(removed.json).sort())===JSON.stringify(['code','device_id','policy_epoch','protocol_version']),'REMOVAL_MINIMAL_KEYS');
 ok(removed.json.code==='DEVICE_REVOKED'&&removed.json.protocol_version===1&&removed.json.device_id===device&&removed.json.policy_epoch===r.json.policy_epoch,'REMOVAL_BOUND_TO_MATCHED_IDENTITY');
 const unknown=await send('/device/sync',{protocol_version:1,after_version:0},randomBytes(32).toString('base64url'));
 ok(unknown.status===401&&JSON.stringify(unknown.json)==='{"code":"UNAUTHORIZED"}','UNKNOWN_CANNOT_DISCOVER_REMOVAL');
 for(const target of [sibling,foreign])ok((await send('/device/sync',{protocol_version:1,after_version:0,device_id:target},credential)).status===400,'REMOVED_TARGET_OVERRIDE_DENIED');
 ok(!JSON.stringify(removed.json).includes(credential),'REMOVAL_NO_SECRET');
 ok(sql(`select count(*) from private.device_credentials where device_id='${device}';`)==='1','REMOVAL_NO_NEW_CREDENTIAL');
 ok(sql(`select count(*) from public.devices where id='${device}';`)==='1','REMOVAL_NO_NEW_DEVICE');
