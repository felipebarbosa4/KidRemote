...
**OD-45 camera/storage continuation — 2026-09-11:** the existing authorization already covers foreground camera enrollment and Keystore identity tests on the owned emulator. The owner now explicitly extends its bounded acceptance work to emulator camera denial/revocation/lifecycle, the documented virtual-scene camera with local synthetic pairing QR, and destructive fault injection only into this application's disposable synthetic ciphertext/key (including a clean-install synthetic restore). Preserve earlier APKs/evidence and keep camera images/secrets local, out of arguments/logs/repository/artifacts. No webcam, physical/OEM acceptance claim, enforcement, rotation, broader removal, deployment or next issue. This is an extension of OD-45, not a duplicate decision or completion of its remaining gates.

**OD-45 single owner-operated physical invalid-camera extension — 2026-09-12:** prospectively approve preparation and ONE owner-operated Samsung SM-X400 camera check of the nonsecret QR `{}`. The agent must not invoke physical ADB. Verify the published child debug APK; install only if this application ID has no existing installation/data record, otherwise STOP without update/uninstall/data or permission change. Owner may manually grant camera permission for this new installation. Explicitly select/verify Samsung model/API; retain no serial, frames, raw logs or decoded personal content. Require fresh unpaired UI, user-initiated Scan, camera-triggered invalid-QR response and no local identity/pending marker; distinguish owner observations, read-only instrumentation and source-derived no-redemption reasoning. No real pairing, backend, endpoint change, enforcement, KR-003 operation, broader physical/OEM acceptance or next issue. Stop scanning and observe camera release; leave the new app/permission installed, without automatic uninstall/clear/revoke. Historical NOT_PASSED results remain unchanged. This extends OD-45 only; virtual-scene investigation is stopped.

**OD-45 subsequent bounded framing authorization and evidence handoff:** the owner explicitly authorized one additional manual framing attempt in the existing installation, without reinstall/data/permission changes. That attempt is now reported complete: owner transcribed the KidRemote invalid-QR message. It is not authority for further scans. The current authorization is limited to recording that observation and one owner-operated read-only provenance/identity/pending-file readback. No agent physical ADB, installer rerun, camera invocation, state mutation or broader acceptance follows. See the [qualified subsequent observation](test-plans/KR-007-PHYSICAL-INVALID-CAMERA.md#subsequent-owner-observation--recorded-2026-09-12); exact event timestamps remain UNSPECIFIED.

**OD-45 readback handoff completed:** the authorized owner-only query at `2026-09-12T05:41:42.0288136Z` verified current installed APK provenance (14d82db hash) and absence of the two known identity/pending files. The earlier scan timestamp remains UNSPECIFIED; this is later-state evidence only, not a contemporaneous check or full diagnostic completion. No further physical command is requested. [Exact retained output and limits](test-plans/KR-007-PHYSICAL-INVALID-CAMERA.md#completed-later-owner-operated-readback). Remaining AC gates and the exclusion of rotation/other issues remain unchanged.
...
**OD-45 prospective AC-6 local rotation extension — 2026-09-12:** the owner now authorizes device credential rotation and response-loss recovery through the actual task-owned local PostgreSQL/gateway and existing explicitly targeted emulator child app. Preserve 90-day expiry, day-30 online renewal, durable old/new candidates and operation ID before server mutation, two-phase five-minute overlap, possession confirmation and explicit expiry recovery. Permit controlled synthetic clocks/committed-response withholding and restart tests, never host/device clock changes. This supersedes only the prior local rotation exclusion; no physical device operations, enforcement, deployment, real use/distribution, AC-7 broader removal or other issue. Preserve all physical invalid-QR evidence and earlier authorizations; PR #20 remains stacked/unmerged. No automatic completion of all KR-007.
