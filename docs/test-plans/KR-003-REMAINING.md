...
| Active-oracle calibration | A,B,C,D | Three source-`5a46f75` runs remain INVALID at runner Accessibility verification; runner-v2 and four runner-v4 attempts passed that correction but stopped at a host-side ARM boundary before attachment/hold; v5 corrects the established same-scope alias | Run the corrected calibration once, including one visible agreement response; stop again on FAIL/INVALID |
