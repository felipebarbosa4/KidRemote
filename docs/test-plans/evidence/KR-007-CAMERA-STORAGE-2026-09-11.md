...
## Focused follow-up — 2026-09-12 UTC

The [separate sanitized boundary record](KR-007-CAMERA-BOUNDARY-2026-09-12.json)
preserves two isolated attempts; neither replaces the historical 23-PASS/one-failure
run below. Same owned AVD/configuration; no backend, real device, frame capture,
webcam, pose manipulation or full-suite local rerun. Investigation stopped after
the single camera-boundary invocation, within the approximately 20-minute tooling
budget excluding build/CI waits.

**OBSERVED:** source `14d82db` APKs opened the camera, bound once, delivered **922
analyzer frames**, converted **922** Y planes, decoded **zero** QR strings, and invoked
neither model callback nor schema rejection. No bind/conversion exception was counted.
The test ended `COMPOSE_TIMEOUT`, with scanning UI still present and no invalid-QR
state. This establishes the last successful boundary as conversion; QR recognition
was not established. Conversion counters do not establish optical readability or
image correctness. No recognizable QR -> continued scanning is not itself a defect;
decoded invalid payload -> rejection remains required and unchanged.

The same retained 512x512 nonsecret PNG generated by `pairingBitmap` decoded to the
expected invalid payload and was rejected by the unchanged schema in a **separate
synthetic fixture-control test**, not camera evidence. Its first invocation failed:
an independent byte round-trip probe found binary stdin to native `adb.exe` delivered
only 5/2787 bytes. The narrow control used native `adb push` to a uniquely task-owned
temporary file, then `run-as` copy; exact byte equality/hash was verified before the
passing fixture test. Temporary staging contained only the nonsecret invalid PNG
and was removed. No credential transport was changed. Exact stdin truncation
mechanism is **UNSPECIFIED**. This new harness defect does not explain the historical
virtual-scene failure, which loads the Windows source PNG directly. Historical
clean-install ciphertext rejection also does not independently establish byte-identical
transport; retain that limitation without rewriting its observed rejection.

**INFERRED:** virtual-scene framing is a plausible stimulus problem, not a reproduced
application decoder defect. [Official virtual-scene guidance](https://developers.google.com/ar/develop/java/emulator#add_augmented_images_to_the_scene)
places wall/table images through the door behind the camera's initial position and
documents interactive camera movement. [Android camera documentation](https://developer.android.com/studio/run/emulator-use-camera)
supports PNG/JPEG QR import, not the inference that loading a poster puts it in view.
This headless run used the default pose. The console accepted both poster commands;
actual QR presence in the camera field remains **UNSPECIFIED**. No unsupported pose
command, image inspection or replacement simulator was introduced.

**Change:** debug-only in-memory integer stage counters, safe exception enums and one
focused instrumentation test; release inline no-op plus DEX counter exclusion audit.
No parser/authentication, enrollment or recognition rule was changed. Separate fixture
test has safe load/decode/schema milestones added at `8f8965f`; those additional
milestones were not exercised on the `14d82db` APK. Local security checks include
counter isolation and no decoder injection in the camera test. CI
[34662155724](https://github.com/felipebarbosa4/KidRemote/actions/runs/34662155724)
passed all five jobs for the exercised source (Android build/lint/JVM/release isolation,
SQL/Auth and native PowerShell 5.1/7). The initial local security failure detected
fixture decoder injection placed in the camera test file; it was separated before
execution. Superseded CI 34662118348 was cancelled, not passed.
Follow-up source `8f8965f` passed all five jobs in
[34662645915](https://github.com/felipebarbosa4/KidRemote/actions/runs/34662645915).
Final local security tests: seven PASS; repository validation and diff whitespace
check PASS. One new static test initially falsely matched `state.message` as
`e.message`; adding the identifier boundary corrected the test, not application code.

Cleanup verified app-data clearing, default poster restoration, temporary-file
removal and targeted AVD stop; zero listeners on 5584/5585. Failed/partial records
and earlier APKs remain retained; media stays owner-local. No fresh valid camera
QR/redemption/repeated-frame/restart test was reached. AC-2/3/4 remain partial;
physical/OEM and AC-6/7 remain pending.

**One next action, proposed only:** a separately authorized brief physical-camera
check on an explicitly selected disposable test installation: scan the nonsecret
invalid QR and require `QR inválido`, using the same safe counters if needed.
This would separate real-camera acquisition from virtual-scene framing; it is not
authorization to install on Samsung, nor proof of valid camera enrollment. No further
emulator tooling or full enrollment rerun is requested here.
