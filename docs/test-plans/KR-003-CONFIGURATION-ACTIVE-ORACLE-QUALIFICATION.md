...
7. After attempt 100, retain one final visible persistence and guided Home/Settings/recovery/re-entry session, followed by automated CLEAR/input verification.
...
- **FAIL:** a physical failure, missing/lost restriction, input delivery or focus regain while blocked, service/permission loss after initial establishment, unsafe recovery result, metric corruption, p95 above 2,000 ms, or failed required cleanup/restoration.
