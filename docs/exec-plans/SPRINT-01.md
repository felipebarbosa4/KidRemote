...
**2026-09-13 recovery:** preserved and inspected the dirty `36f85a4` worktree. Prior SQL/HTTP checkpoint and failed Windows CI are retained in the [AC-6 evidence](../test-plans/evidence/KR-007-ROTATION-2026-09-12.md). Android rotation had no retained execution report; continue its actual local runtime and the three pending HTTP negatives only, with required regressions/CI.

**AC-6 result:** source `3202a5e` passed 521 SQL, 44 Auth, 29 enrollment HTTP, 27 rotation HTTP and 24 actual Android invocations (nine AC-6, fifteen regressions). All five source CI jobs passed. Encrypted candidates survived BEGIN/CONFIRM response loss, restart and pending gateway outage; expired/revoked contacts denied with identity retained. Cleanup/AVD stop verified. Local ports changed only to avoid Windows reservations. AC-6 locally demonstrated; remaining AC-2/3/4/7 and physical/configured-policy gaps stay open. Exact attempts/APK hashes are in the evidence above.
