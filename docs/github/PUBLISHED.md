...
The [immutable visual-calibration bundle](../test-plans/evidence/KR-003-SAMSUNG-VISUAL-CALIBRATION-BUNDLE-2026-09-10.md) is published from source `5150e720b99a6db1f87607eed8e42ee483c17b2c` at `C:\platform-tools\kr003-visual-calibration-bundles\5150e72`; its `bundle.json` SHA-256 is `c602106ec2335860742ba88fd1d9f96274f666ce37b3931f14516ba73b9a4c89`. Physical execution is **Not run**. The exact mounted payload hashes pass, the actual entrypoint and standalone bailout were exercised with redirected/fake input under native Windows PowerShell 5.1 without a device command, and local Node, repository, Android build/lint and release-isolation checks passed. Exact-source PowerShell 7/native Windows and Android CI are pending at publication-record commit time.

The earlier `d303082` packaging artifact was rejected before handoff because its standalone CLEAR helper did not yet allowlist the visual protocol. The later `03f4bc2` artifact was superseded before handoff after CI found test-harness portability assumptions; its runner payload is unchanged, but only `5150e72` is recommended. All remain unmodified. No Samsung command was run. Immutable full runner-v12 `80dcdf4` is unchanged and not requested for execution. KR-003 remains Open/In Progress, draft PR #16 remains Open, and KR-004 remains untouched.
