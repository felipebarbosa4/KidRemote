begin;
-- Latest immutable report retry identity; no event/package history.
alter table public.device_state add column report_digest bytea check(octet_length(report_digest)=32);
create function public.accept_device_report(p_digest bytea,p_report jsonb) returns jsonb
...
begin
 ...
 p_report->'restriction_applied'<>'false'::jsonb or p_report->>'health'<>'ENFORCEMENT_UNAVAILABLE' or
 ...
 values(d.id,d.household_id,d.policy_epoch,ver,seq,period,used,bonus,remaining,manual,required,false,'ENFORCEMENT_UNAVAILABLE:'||(p_report->>'accounting_status'),(p_report->>'observed_at')::timestamptz,received,hash)
 on conflict(device_id) do update set applied_version=excluded.applied_version,report_sequence=excluded.report_sequence,period_key=excluded.period_key,used_ms=excluded.used_ms,bonus_seconds=excluded.bonus_seconds,remaining_ms=excluded.remaining_ms,manual_lock=excluded.manual_lock,restriction_required=excluded.restriction_required,restriction_applied=false,health=excluded.health,observed_at=excluded.observed_at,received_at=excluded.received_at,report_digest=excluded.report_digest;
...
create view public.operation_status with (security_invoker=true) as
 ...
 when r.outcome='applied' and r.observed_enforcement and s.restriction_applied then 'applied'
...
-- PostgREST 14 retries custom 40001 forever. Preserve KR-004's SQL contract;
-- translate only its known business conflicts at the authenticated HTTP RPC boundary.
create function public.parent_operation(p_operation_id uuid,p_device_id uuid,p_kind text,p_payload jsonb,p_expected_version bigint default null)
returns jsonb language plpgsql security invoker set search_path='' as $rpc$
begin
 return public.accept_control(p_operation_id,p_device_id,p_kind,p_payload,p_expected_version);
exception when serialization_failure then
 if sqlerrm in ('OPERATION_CONFLICT','VERSION_CONFLICT','PERIOD_CONFLICT') then raise exception using errcode='PT409',message=sqlerrm;end if;
 raise;
end $rpc$;
revoke all on function public.parent_operation(uuid,uuid,text,jsonb,bigint) from public,anon,authenticated,service_role;
grant execute on function public.parent_operation(uuid,uuid,text,jsonb,bigint) to authenticated;
commit;
