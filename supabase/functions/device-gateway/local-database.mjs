...
export function databaseRepository(session) {
 return {rotate:async(d,p)=>session(async query=>{
  ...
  try {
   ...
   const out=await callback({...row,report:async body=>JSON.parse(await query(`select public.accept_device_report(${digest(d)},${literal(JSON.stringify(body))}::jsonb);`)),sync:async(scope,{after_version})=>{
    if(policy.policy_configured===true) {
     ...
     const snapshot=JSON.parse(await query(`with t as (select clock_timestamp() as utc), h as (select * from public.households where id='${scope.household_id}'::uuid),
      ops as (select c.id,c.version,c.kind,c.period_key,s.status from public.commands c join public.operation_status s on s.operation_id=c.id where c.device_id='${scope.device_id}'::uuid and c.version>${after_version} and c.version<=${policy.version} order by c.version desc limit 100)
      ...
      'history_pruned',(select count(*)>100 from public.commands where device_id='${scope.device_id}'::uuid and version>${after_version} and version<=${policy.version})) from h,t;`));
     return {protocol_version:1,kind:'CONFIGURED_SNAPSHOT',device_id:scope.device_id,policy_epoch:scope.policy_epoch,version:policy.version,
